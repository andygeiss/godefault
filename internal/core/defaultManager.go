...
actor User as U
{{ $usesMessageBus := hasMethodContains . "MessageBus" }}{{ if $usesMessageBus }}{{ println }}participant MessageBus as B #E040FB{{ end }}{{ range $s := .Structs }}{{ $hasSuffixClient := endsWith $s.Name "Client" }}{{ $hasSuffixManager := endsWith $s.Name "Manager" }}{{ $hasSuffixEngine := endsWith $s.Name "Engine" }}{{ $hasSuffixResourceAccess := endsWith $s.Name "Access" }}
